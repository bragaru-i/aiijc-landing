...
const useAvatar = (avatarUrl, patchUrl) => {
    ...
    const inputChange = async (event) => {
        ...
        if (!checkSize(uploadedImage)) {
            showError(`${t('Profile.Avatar.onFileLimit1')}${MAX_SIZE_MB}MB. ${t('Profile.Avatar.onFileLimit2')}. `)
            closeBoxes();
            return;
        }
    ...
    const deleteImage = async () => {
        ...
        try {
            const response = await fetch(patchUrl, {
                ...
                body: JSON.stringify({avatar: null, cropping:null})
            });
    ...
    const saveImageHandler = async (cropedImage, rotateImage) => {
        ...
        // full image not saved to db, that=s why we automatic crop that image, and also not saving image
        if (image) data.avatar = image;
   
        let res = null;
        try {
            res = await api.patchFormData(patchUrl, data);
