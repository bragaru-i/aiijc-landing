...
const User = ({participant, request, currentUser, user, team, createRequest, deleteRequest, number}) => {
    ...
    }, [request, user, team, deleteRequest]);
...
const Users = ({user: currentUser, team, setInvitationsCounter}) => {
    const {t} = useTranslation();
    ...
    return (
        <>
            <Helmet title={t("teams.search")}/>
            <div className={s.headingWr}>
                <Heading text={t("teams.search")} className={s.heading}/>
                {requestsLoaded && (
            ...
            <Card>
                {(!content || isLoading) ? (
                    ...
                    <>
                        <SearchForm initialAge={age} initialSearch={search}
                                    onSubmit={onSubmit}/>

                        <div className={s.header}>
                            <div>
                                {content.results.length > 0 ? (
                                    <> {t('teams.SearchUsers.foundCnt')}: <span className={s.count}>{content.count}</span></>
                                ) : (
                                    t('teams.SearchUsers.noUsers')
                                )}
                            </div>
                            <button className={s.reset} onClick={resetFilters}>{t('teams.SearchUsers.resetFilters')}</button>
                        </div>

                        {content.results.length > 0 && (
                            <>
                                <ol className={baseStyles.users}>
                                    {content.results.map((user, index) => (
                                        <User user={user}
                                              team={team}
                                              createRequest={createRequest}
                                              deleteRequest={deleteRequest}
                                              request={usersToRequests.get(user.id)}
                                              participant={usersToParticipants.get(user.id)}
                                ...
                                <div className={cn("ratingFooter", baseStyles.onlyDesktop)}>
                                    <Paginator numPages={Math.ceil(content.count / filterPerPage)}
                                               currentPage={currentPage}
                                               setCurrentPage={setCurrentPage} isLoading={isLoading}/>

                                    <div className="pageFilter">
                                        <span className="pageFilterLabel">{t('Rating.filterPerPage')}</span>
                                        {availableFilterPerPage.map((currentFilterPerPage) => (
                                            <span key={currentFilterPerPage}
                                                  className={cn({
                                                      active: filterPerPage === currentFilterPerPage,
                                                      disabled: isLoading
                                                  })}
                                                  onClick={() => {
                                                      setFilters({page: 1, size: currentFilterPerPage});
                                                  }}>
                                                {currentFilterPerPage}
                                            </span>
                                        ))}
                                    </div>
                                </div>
